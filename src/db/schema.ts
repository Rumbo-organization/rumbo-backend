...
export const contactKind = pgEnum('contact_kind', ['PERSONA_FISICA', 'PERSONA_JURIDICA']);
...
export const contactStatus = pgEnum('contact_status', ['prospecto', 'asegurado', 'exasegurado']);
...
// la etapa se limpia.
export const prospectStage = pgEnum('prospect_stage', ['nuevo', 'contactado', 'cotizado', 'negociacion']);
...
export const recordSource = pgEnum('record_source', ['manual', 'import_csv', 'sync']);
...
export const producerCodes = pgTable(
  ...
  table => [
    ...
    uniqueIndex('producer_codes_org_insurer_code_idx').on(table.orgId, table.insurerId, table.code),
  ],
);
...
export const policyStatus = pgEnum('policy_status', ['propuesta', 'vigente', 'vencida', 'anulada', 'renovada']);

// Forma de pago de la póliza (agregada en migración posterior al checkout base).
export const paymentMethod = pgEnum('payment_method', ['cupon', 'debito_bancario', 'tarjeta_credito']);

// Centro de Siniestros (M4, F-030/F-032). Estados default; custom states → v0.3.
export const claimStatus = pgEnum('claim_status', ['abierto', 'en_curso', 'cerrado']);
...
export const claimEventKind = pgEnum('claim_event_kind', ['comment', 'status_change']);
...
export const claimImportance = pgEnum('claim_importance', ['alta', 'media', 'baja']);
...
export const policies = pgTable(
  'policies',
  {
    ...
    renewedFromPolicyId: uuid('renewed_from_policy_id').references((): AnyPgColumn => policies.id, {
      onDelete: 'set null',
    }),
...
export const policyEndorsements = pgTable(
  ...
  table => [
    ...
    uniqueIndex('policy_endorsements_policy_number_idx').on(table.orgId, table.policyId, table.number),
  ],
);
...
export const policyParties = pgTable(
  ...
  table => [
    ...
    uniqueIndex('policy_parties_unique_idx').on(table.orgId, table.policyId, table.contactId, table.role),
  ],
);
...
export const contactRelationships = pgTable(
  ...
  table => [
    ...
    uniqueIndex('contact_relationships_pair_idx').on(table.orgId, table.contactId, table.relatedContactId),
  ],
);
...
export const contactAssignees = pgTable(
  ...
  table => [
    ...
    uniqueIndex('contact_assignees_unique_idx').on(table.orgId, table.contactId, table.userId, table.role),
  ],
);
...
export const quoteItems = pgTable(
  ...
  table => [index('quote_items_org_idx').on(table.orgId), index('quote_items_quote_idx').on(table.quoteId)],
);
...
export const communicationChannel = pgEnum('communication_channel', ['whatsapp', 'email', 'llamada', 'otro']);
...
export const expiryNotificationWindow = pgEnum('expiry_notification_window', ['30d', '7d']);
...
export const expiryNotifications = pgTable(
  ...
  table => [
    uniqueIndex('expiry_notifications_policy_window_idx').on(table.policyId, table.window),
...
export const calendarEventKind = pgEnum('calendar_event_kind', ['llamada', 'reunion', 'tramite', 'otro']);
