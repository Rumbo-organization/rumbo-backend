...
contactExtras.post(
  ...
  wrap(async (req, res) => {
    ...
    if (!isUuid(contactId) || !isUuid(relatedContactId)) {
      res.status(400).json({ error: 'Asegurado inválido.' });
      return;
    }
    if (contactId === relatedContactId) {
      res.status(400).json({ error: 'No se puede relacionar un asegurado consigo mismo.' });
      return;
    }
    if (!RELATION_TYPES.includes(type)) {
      res.status(400).json({ error: 'Tipo de relación inválido.' });
      return;
    }
    ...
    const out = await withAuthedTx(ctx, async tx => {
        ...
        .where(
          or(
            and(
              ...
              eq(contactRelationships.relatedContactId, relatedContactId),
            ),
            and(
              eq(contactRelationships.contactId, relatedContactId),
              eq(contactRelationships.relatedContactId, contactId),
            ),
          ),
        )
        .limit(1);
      ...
      await writeAuditLogTx(tx, {
        orgId: ctx.orgId,
        userId: ctx.userId,
        action: 'create_contact_relationship',
    ...
    if (out === 'exists') {
      res.status(409).json({ error: 'Ya existe una relación entre estas personas.' });
      return;
    }
  ...
  wrap(async (req, res) => {
    ...
    const out = await withAuthedTx(ctx, async tx => {
      ...
      await writeAuditLogTx(tx, {
        orgId: ctx.orgId,
        userId: ctx.userId,
        action: 'delete_contact_relationship',
        entityType: 'contact_relationship',
        entityId: row.id,
      });
      return row;
...
contactExtras.post(
  ...
  wrap(async (req, res) => {
    ...
    const b = (req.body ?? {}) as Record<string, unknown>;
    const s = (k: string, max = 120) =>
      typeof b[k] === 'string' ? (b[k] as string).trim().slice(0, max) || null : null;
    const street = s('street');
    if (!street) {
      res.status(400).json({ error: 'Falta la calle.' });
      return;
    }
    const ctx = req.authCtx!;
    const out = await withAuthedTx(ctx, async tx => {
    ...
    try {
      const out = await withAuthedTx(ctx, async tx => {
        ...
        await writeAuditLogTx(tx, {
          orgId: ctx.orgId,
          userId: ctx.userId,
          action: 'create_contact_assignee',
          entityType: 'contact_assignee',
          entityId: row!.id,
      ...
      if (out === 'no-member') {
        res.status(400).json({ error: 'El usuario no pertenece a la organización.' });
        return;
      }
      res.status(201).json(out);
    } catch (e) {
      if (
        e instanceof Error &&
        /contact_assignees_unique_idx|duplicate key/.test(
          e.message + ' ' + String((e as { cause?: unknown }).cause ?? ''),
        )
      ) {
        res.status(409).json({ error: 'Ese usuario ya está asignado con ese rol.' });
        return;
      }
      throw e;
    }
...
contactExtras.delete(
  ...
  wrap(async (req, res) => {
    ...
    const out = await withAuthedTx(ctx, async tx => {
      const [row] = await tx
        .delete(contactAssignees)
        .where(eq(contactAssignees.id, id))
        .returning({ id: contactAssignees.id });
      if (!row) return null;
      await writeAuditLogTx(tx, {
        orgId: ctx.orgId,
        userId: ctx.userId,
        action: 'delete_contact_assignee',
        entityType: 'contact_assignee',
        entityId: row.id,
      });
      return row;
