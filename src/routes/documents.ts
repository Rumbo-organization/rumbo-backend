...
documentsRouter.post('/documents/upload', upload.single('file'), wrap(async (req, res) => {
  if (!isR2Configured()) { res.status(503).json({ error: 'El almacenamiento de documentos no está configurado todavía (R2).' }); return; }
  const file = req.file;
  if (!file) { res.status(400).json({ error: 'Falta el archivo.' }); return; }
  ...
  if (Boolean(policyId) === Boolean(contactId)) {
    res.status(400).json({ error: 'Indicá una póliza o un asegurado (exactamente uno).' }); return;
  }
  if ((policyId && !isUuid(policyId)) || (contactId && !isUuid(contactId))) {
    res.status(400).json({ error: 'Id inválido.' }); return;
  }
  if (file.size === 0) { res.status(400).json({ error: 'El archivo está vacío.' }); return; }
  const contentType = file.mimetype || 'application/octet-stream';
  if (!ALLOWED.has(contentType)) { res.status(400).json({ error: 'Formato no permitido (PDF o imagen).' }); return; }
  ...
  if (!owned) { res.status(404).json({ error: 'No encontrado.' }); return; }
...
documentsRouter.get('/documents/:id', wrap(async (req, res) => {
