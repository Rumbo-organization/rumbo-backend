...
policyExtras.post('/policies/:id/installments', wrap(async (req, res) => {
  const policyId = req.params.id;
  if (!isUuid(policyId)) { res.status(400).json({ error: 'Id inválido.' }); return; }
  const b = (req.body ?? {}) as Record<string, unknown>;
  const count = Number(b.count);
  const totalAmount = Number(b.totalAmount);
  if (!Number.isInteger(count) || count < 1 || count > 60) { res.status(400).json({ error: 'Cantidad de cuotas inválida (1 a 60).' }); return; }
  if (!isYmd(b.firstDueDate)) { res.status(400).json({ error: 'Primera fecha inválida (AAAA-MM-DD).' }); return; }
  if (!Number.isFinite(totalAmount) || totalAmount <= 0) { res.status(400).json({ error: 'Importe total inválido.' }); return; }
  ...
  const out = await withAuthedTx(ctx, async (tx) => {
    ...
    const [existing] = await tx.select({ id: policyInstallments.id }).from(policyInstallments).where(eq(policyInstallments.policyId, policyId)).limit(1);
    if (existing) return 'exists';
    await tx.insert(policyInstallments).values(
      buildInstallments(count, b.firstDueDate as string, totalAmount).map((p) => ({
        orgId: ctx.orgId, policyId, number: p.number, dueDate: p.dueDate, amount: p.amount, source: 'manual' as const,
      })),
    );
    await writeAuditLogTx(tx, {
      orgId: ctx.orgId, userId: ctx.userId, action: 'generate_installments', entityType: 'policy', entityId: policyId,
      payload: { count },
    });
    return 'ok';
  });
  if (out === 'not-found') { res.status(404).json({ error: 'Póliza no encontrada.' }); return; }
  if (out === 'exists') { res.status(409).json({ error: 'La póliza ya tiene plan de pagos. Rehacelo borrándolo primero.' }); return; }
  res.status(201).json({ ok: true });
}));

// Marca/desmarca una cuota como pagada.
policyExtras.patch('/installments/:id', wrap(async (req, res) => {
  ...
  const out = await withAuthedTx(ctx, async (tx) => {
    ...
    if (!row) return null;
    await writeAuditLogTx(tx, {
      orgId: ctx.orgId, userId: ctx.userId, action: 'set_installment_paid', entityType: 'installment', entityId: row.id,
      payload: { paid },
    });
    return row;
  });
  if (!out) { res.status(404).json({ error: 'Cuota no encontrada.' }); return; }
  res.json({ ok: true });
}));

// Borra el plan entero (para rehacerlo).
policyExtras.delete('/policies/:id/installments', wrap(async (req, res) => {
  const policyId = req.params.id;
  if (!isUuid(policyId)) { res.status(400).json({ error: 'Id inválido.' }); return; }
  const ctx = req.authCtx!;
  await withAuthedTx(ctx, async (tx) => {
    ...
    await writeAuditLogTx(tx, {
      orgId: ctx.orgId, userId: ctx.userId, action: 'clear_installments', entityType: 'policy', entityId: policyId,
    });
  });
  res.json({ ok: true });
}));
...
policyExtras.get('/policies/:id/endorsements', wrap(async (req, res) => {
  const policyId = req.params.id;
  if (!isUuid(policyId)) { res.status(400).json({ error: 'Id inválido.' }); return; }
...
policyExtras.post('/policies/:id/endorsements', wrap(async (req, res) => {
  const policyId = req.params.id;
  if (!isUuid(policyId)) { res.status(400).json({ error: 'Id inválido.' }); return; }
  const b = (req.body ?? {}) as Record<string, unknown>;
  const number = Number(b.number);
  const type = String(b.type ?? '');
  if (!Number.isInteger(number) || number < 0) { res.status(400).json({ error: 'Número de movimiento inválido.' }); return; }
  if (!ENDORSEMENT_TYPES.includes(type)) { res.status(400).json({ error: 'Tipo de movimiento inválido.' }); return; }
  if (b.issuedAt != null && b.issuedAt !== '' && !isYmd(b.issuedAt)) { res.status(400).json({ error: 'Fecha de emisión inválida.' }); return; }
  ...
  try {
    const out = await withAuthedTx(ctx, async (tx) => {
      ...
      await writeAuditLogTx(tx, {
        orgId: ctx.orgId, userId: ctx.userId, action: 'create_endorsement', entityType: 'policy_endorsement', entityId: row!.id,
    ...
    if (e instanceof Error && /duplicate key/.test(e.message + ' ' + String((e as { cause?: unknown }).cause ?? ''))) {
      res.status(409).json({ error: 'Ya existe un movimiento con ese número en la póliza.' }); return;
    }
    throw e;
  }
}));

policyExtras.delete('/endorsements/:id', wrap(async (req, res) => {
  const id = req.params.id;
  if (!isUuid(id)) { res.status(400).json({ error: 'Id inválido.' }); return; }
  const ctx = req.authCtx!;
  const out = await withAuthedTx(ctx, async (tx) => {
    const [row] = await tx.delete(policyEndorsements).where(eq(policyEndorsements.id, id)).returning({ id: policyEndorsements.id });
    if (!row) return null;
    await writeAuditLogTx(tx, {
      orgId: ctx.orgId, userId: ctx.userId, action: 'delete_endorsement', entityType: 'policy_endorsement', entityId: row.id,
    });
    return row;
  });
  if (!out) { res.status(404).json({ error: 'Movimiento no encontrado.' }); return; }
  res.json({ ok: true });
}));

// ── Personas de la póliza ─────────────────────────────────────────────────────

policyExtras.get('/policies/:id/parties', wrap(async (req, res) => {
  const policyId = req.params.id;
  if (!isUuid(policyId)) { res.status(400).json({ error: 'Id inválido.' }); return; }
...
policyExtras.post('/policies/:id/parties', wrap(async (req, res) => {
  const policyId = req.params.id;
  if (!isUuid(policyId)) { res.status(400).json({ error: 'Id inválido.' }); return; }
  const b = (req.body ?? {}) as Record<string, unknown>;
  const contactId = String(b.contactId ?? '');
  const role = String(b.role ?? '');
  if (!isUuid(contactId)) { res.status(400).json({ error: 'Asegurado inválido.' }); return; }
  if (!PARTY_ROLES.includes(role)) { res.status(400).json({ error: 'Rol inválido.' }); return; }
  const ctx = req.authCtx!;
  try {
    const out = await withAuthedTx(ctx, async (tx) => {
      const [policy] = await tx.select({ id: policies.id }).from(policies).where(eq(policies.id, policyId)).limit(1);
      ...
      const [row] = await tx
        ...
        .values({
          orgId: ctx.orgId, policyId, contactId,
          role: role as (typeof policyParties.$inferInsert)['role'],
          source: 'manual' as const,
        })
        .returning({ id: policyParties.id, role: policyParties.role });
      await writeAuditLogTx(tx, {
        orgId: ctx.orgId, userId: ctx.userId, action: 'create_policy_party', entityType: 'policy_party', entityId: row!.id,
        payload: { policyId, role: row!.role },
      });
      return row;
    });
    if (out === 'no-policy') { res.status(404).json({ error: 'Póliza no encontrada.' }); return; }
    if (out === 'no-contact') { res.status(404).json({ error: 'Asegurado no encontrado.' }); return; }
    res.status(201).json(out);
  } catch (e) {
    if (e instanceof Error && /policy_parties_unique_idx|duplicate key/.test(e.message + ' ' + String((e as { cause?: unknown }).cause ?? ''))) {
      res.status(409).json({ error: 'Esa persona ya figura con ese rol en la póliza.' }); return;
    }
    throw e;
  }
}));

policyExtras.delete('/parties/:id', wrap(async (req, res) => {
  const id = req.params.id;
  if (!isUuid(id)) { res.status(400).json({ error: 'Id inválido.' }); return; }
  const ctx = req.authCtx!;
  const out = await withAuthedTx(ctx, async (tx) => {
    ...
    if (!row) return null;
    await writeAuditLogTx(tx, {
      orgId: ctx.orgId, userId: ctx.userId, action: 'delete_policy_party', entityType: 'policy_party', entityId: row.id,
    });
    return row;
  });
  if (!out) { res.status(404).json({ error: 'Persona no encontrada.' }); return; }
  res.json({ ok: true });
}));
