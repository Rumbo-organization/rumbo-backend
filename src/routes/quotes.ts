...
const POLICY_RAMOS = [
  'automotor', 'motovehiculo', 'hogar', 'vida', 'art', 'comercio', 'accidentes_personales',
  'incendio', 'responsabilidad_civil', 'consorcio', 'seguro_tecnico', 'transporte', 'embarcaciones', 'otros',
];
...
function displayName(c: { kind: string | null; firstName: string | null; lastName: string | null; legalName: string | null }): string {
...
quotesRouter.get('/cotizaciones', wrap(async (req, res) => {
  ...
  const out = await withAuthedTx(req.authCtx!, async (tx) => {
    const rows = await tx
      .select({
        q: quotes,
        cKind: contacts.kind, cFirst: contacts.firstName, cLast: contacts.lastName, cLegal: contacts.legalName,
        itemCount: sql<number>`(select count(*)::int from ${quoteItems} where ${quoteItems.quoteId} = ${quotes.id})`,
        bestCuota: sql<string | null>`(select ${quoteItems.cuota} from ${quoteItems} where ${quoteItems.quoteId} = ${quotes.id} and ${quoteItems.cuota} is not null order by ${quoteItems.cuota} asc limit 1)`,
        bestInsurer: sql<string | null>`(select ${insurers.name} from ${quoteItems} join ${insurers} on ${insurers.id} = ${quoteItems.insurerId} where ${quoteItems.quoteId} = ${quotes.id} and ${quoteItems.cuota} is not null order by ${quoteItems.cuota} asc limit 1)`,
      })
      .from(quotes)
...
quotesRouter.get('/quotes/:id', wrap(async (req, res) => {
  ...
  const out = await withAuthedTx(req.authCtx!, async (tx) => {
    ...
    const client = displayName({ kind: row.cKind, firstName: row.cFirst, lastName: row.cLast, legalName: row.cLegal });
    return {
      id: q.id,
      num: quoteNum(q.id, q.createdAt),
  ...
  if (!out) { res.status(404).json({ error: 'Cotización no encontrada.' }); return; }
  res.json(out);
}));

quotesRouter.post('/quotes', wrap(async (req, res) => {
  const b = (req.body ?? {}) as Record<string, unknown>;
  const contactId = b.contactId != null && b.contactId !== '' ? String(b.contactId) : null;
  if (contactId !== null && !isUuid(contactId)) { res.status(400).json({ error: 'Asegurado inválido.' }); return; }
  ...
  const out = await withAuthedTx(ctx, async (tx) => {
    ...
    await writeAuditLogTx(tx, {
      orgId: ctx.orgId, userId: ctx.userId, action: 'create_quote', entityType: 'quote', entityId: row!.id,
      payload: { ramo },
    });
    return row;
  });
...
quotesRouter.delete('/quotes/:id', wrap(async (req, res) => {
  const id = req.params.id;
  if (!isUuid(id)) { res.status(400).json({ error: 'Id inválido.' }); return; }
  const ctx = req.authCtx!;
  const out = await withAuthedTx(ctx, async (tx) => {
    ...
    if (!row) return null;
    await writeAuditLogTx(tx, {
      orgId: ctx.orgId, userId: ctx.userId, action: 'delete_quote', entityType: 'quote', entityId: row.id,
    });
    return row;
  });
  if (!out) { res.status(404).json({ error: 'Cotización no encontrada.' }); return; }
...
quotesRouter.post('/quotes/:id/items', wrap(async (req, res) => {
  const quoteId = req.params.id;
  if (!isUuid(quoteId)) { res.status(400).json({ error: 'Id inválido.' }); return; }
  const b = (req.body ?? {}) as Record<string, unknown>;
  const insurerId = String(b.insurerId ?? '');
  if (!isUuid(insurerId)) { res.status(400).json({ error: 'Elegí una aseguradora.' }); return; }
  ...
  if ((suma == null || !Number.isFinite(suma)) && (cuota == null || !Number.isFinite(cuota))) {
    res.status(400).json({ error: 'Cargá al menos la cuota o la suma asegurada.' }); return;
  }
  const ctx = req.authCtx!;
  const out = await withAuthedTx(ctx, async (tx) => {
    ...
    await writeAuditLogTx(tx, {
      orgId: ctx.orgId, userId: ctx.userId, action: 'add_quote_item', entityType: 'quote', entityId: quoteId,
      payload: { insurerId, coverage: row!.coverage },
    });
    return row;
  });
  if (out === 'no-quote') { res.status(404).json({ error: 'Cotización no encontrada.' }); return; }
  if (out === 'no-insurer') { res.status(404).json({ error: 'Aseguradora no encontrada.' }); return; }
  res.status(201).json(out);
}));

quotesRouter.delete('/quote-items/:id', wrap(async (req, res) => {
  const id = req.params.id;
  if (!isUuid(id)) { res.status(400).json({ error: 'Id inválido.' }); return; }
  const ctx = req.authCtx!;
  const out = await withAuthedTx(ctx, async (tx) => {
    const [row] = await tx.delete(quoteItems).where(eq(quoteItems.id, id)).returning({ id: quoteItems.id, quoteId: quoteItems.quoteId });
    if (!row) return null;
    await writeAuditLogTx(tx, {
      orgId: ctx.orgId, userId: ctx.userId, action: 'delete_quote_item', entityType: 'quote', entityId: row.quoteId,
    });
    return row;
  });
  if (!out) { res.status(404).json({ error: 'Opción no encontrada.' }); return; }
  res.json({ ok: true });
}));
...
quotesRouter.get('/insurers/picker', wrap(async (req, res) => {
...
quotesRouter.post('/insurers', wrap(async (req, res) => {
  const name = optionalText((req.body ?? {}).name, 120);
  if (!name) { res.status(400).json({ error: 'Falta el nombre.' }); return; }
  const ctx = req.authCtx!;
  const out = await withAuthedTx(ctx, async (tx) => {
    const [existing] = await tx.select({ id: insurers.id }).from(insurers)
      .where(sql`lower(${insurers.name}) = lower(${name})`).limit(1);
    if (existing) return existing;
    const [row] = await tx.insert(insurers).values({ orgId: ctx.orgId, name }).returning({ id: insurers.id });
    await writeAuditLogTx(tx, {
      orgId: ctx.orgId, userId: ctx.userId, action: 'create_insurer', entityType: 'insurer', entityId: row!.id,
      payload: { name },
    });
    return row;
  });
